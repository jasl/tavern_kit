...
#
class Conversation::CopilotCandidateGenerator
  MAX_CANDIDATES = 4
  ...
  def call
    ...
    Rails.logger.error "[Conversation::CopilotCandidateGenerator] Prompt build failed: #{e.message}"
    broadcast_error(e.message)
  rescue SimpleInference::Errors::HTTPError => e
    Rails.logger.error "[Conversation::CopilotCandidateGenerator] API error: #{e.message}"
    broadcast_error("API error: #{e.message}")
  rescue SimpleInference::Errors::TimeoutError => e
    Rails.logger.error "[Conversation::CopilotCandidateGenerator] Timeout error: #{e.message}"
    broadcast_error("Request timed out")
  rescue SimpleInference::Errors::ConnectionError => e
    Rails.logger.error "[Conversation::CopilotCandidateGenerator] Connection error: #{e.message}"
  ...
  def generate_single_candidate(client, messages, index)
    ...
    Rails.logger.error "[Conversation::CopilotCandidateGenerator] Candidate #{index} failed: #{e.message}"
  end
