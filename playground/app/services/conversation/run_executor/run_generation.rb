...
#
class Conversation::RunExecutor::RunGeneration
  ...
  def generate_response(prompt_messages)
    ...
    raise Conversation::RunExecutor::Canceled if cancel_requested?(force: true)
    ...
    if @llm_client.provider&.streamable? && streaming_enabled?
      ...
      @llm_client.chat(**gen_params) do |chunk|
        raise Conversation::RunExecutor::Canceled if cancel_requested?
  ...
  def cancel_requested?(force: false)
    ...
    unless force
      interval = Conversation::RunExecutor::CANCEL_POLL_INTERVAL_SECONDS
  ...
  def touch_run_heartbeat!(force: false)
    ...
    now = Process.clock_gettime(Process::CLOCK_MONOTONIC)
    unless force
      interval = Conversation::RunExecutor::HEARTBEAT_INTERVAL_SECONDS
