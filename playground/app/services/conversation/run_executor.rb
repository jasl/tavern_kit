...
class Conversation::RunExecutor
  ...
  def execute!
    ...
    @run = Conversation::RunExecutor::RunClaimer.new(run_id: run_id).claim!
    return unless run
    ...
    persistence = Conversation::RunExecutor::RunPersistence.new(run: run, conversation: conversation, space: space, speaker: speaker)
    ...
    generation = Conversation::RunExecutor::RunGeneration.new(run: run, conversation: conversation, speaker: speaker)
    ...
    Conversation::RunExecutor::RunFollowups
