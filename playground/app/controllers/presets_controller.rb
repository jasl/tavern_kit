...
class PresetsController < ApplicationController
  ...
  def import
    ...
    files.each do |file|
      result = detector.execute(file, user: Current.user)
      if result.success?
    ...
    if files.one?
      if failures.empty?
        redirect_to presets_path, notice: t("presets.imported", default: "Preset '%{name}' imported successfully.", name: successes.first.name)
      else
        redirect_to presets_path, alert: t("presets.import_failed", default: "Import failed: %{error}", error: failures.first[:error])
      end
      return
    end

    if failures.empty?
      redirect_to presets_path, notice: t("presets.imported_many", default: "Imported %{count} presets.", count: successes.size)
      return
    end
    ...
    redirect_to presets_path, flash: flash
  end
