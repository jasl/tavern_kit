...
module Characters
  # Controller for managing embedded lorebook entries (character_book.entries) for user-owned characters.
  #
  ...
  #
  # Access control: Only the character owner can access this controller.
  #
  class EmbeddedLorebookEntriesController < ApplicationController
    ...
    # GET /characters/:character_id/embedded_lorebook_entries/new
    def new
      @entry = EmbeddedLorebookEntryPresenter.new(default_entry_attributes, character: @character, mode: :public)
    end

    # POST /characters/:character_id/embedded_lorebook_entries
    def create
      ...
      if save_embedded_entries(entries)
        ...
        @entry = EmbeddedLorebookEntryPresenter.new(entry_attrs, character: @character, mode: :public)
        flash.now[:alert] = @character.errors.full_messages.to_sentence
        render :new, status: :unprocessable_entity
      end
    end

    # GET /characters/:character_id/embedded_lorebook_entries/:id/edit
    def edit
    end

    # PATCH/PUT /characters/:character_id/embedded_lorebook_entries/:id
    def update
      entry_attrs = entry_params_to_hash
      ...
      if save_embedded_entries(entries)
        respond_to do |format|
          ...
          format.turbo_stream do
            ...
            @entry = EmbeddedLorebookEntryPresenter.new(updated_entry, character: @character, mode: :public)
            render turbo_stream: turbo_stream.action(
              :replace,
              @entry.dom_id,
              render_to_string(
                partial: "characters/embedded_lorebook_entries/inline_entry_row",
                locals: { presenter: @entry, character: @character, mode: :public, editable: true }
              ),
              method: "morph"
            )
          end
        ...
        @entry = EmbeddedLorebookEntryPresenter.new(entry_attrs.merge(id: params[:id]), character: @character, mode: :public)
    ...
    # DELETE /characters/:character_id/embedded_lorebook_entries/:id
    def destroy
      ...
      if save_embedded_entries(entries)
            ...
            if params[:inline].present?
              ...
              render turbo_stream: turbo_stream.action(
                ...
                render_to_string(
                  ...
                  locals: { character: @character, mode: :public }
                ),
                method: "morph"
              )
            else
              redirect_to edit_character_path(@character), status: :see_other
            end
          end
        end
      else
        ...
        respond_to do |format|
          format.html { redirect_to edit_character_path(@character), alert: error_message }
          format.turbo_stream do
            render_toast_turbo_stream(message: error_message, type: "error", status: :unprocessable_entity)
          end
        end
    ...
    # PATCH /characters/:character_id/embedded_lorebook_entries/reorder
    def reorder
    ...
    def set_character
      @character = Character.accessible_to(Current.user).find(params[:character_id])
    end

    def require_owner
      return if @character.user_id == Current.user&.id
    ...
    def set_entry
      ...
      @entry = EmbeddedLorebookEntryPresenter.new(entry_hash, character: @character, mode: :public)
    end
    ...
    def ensure_character_unlocked
      ...
      respond_to do |format|
        format.html do
          redirect_to edit_character_path(@character), alert: error_message
        end
        format.turbo_stream do
