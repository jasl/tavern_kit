...
# - human_turn: Tracks human's turn in auto mode (can be skipped)
...
class ConversationRun < ApplicationRecord
  ...
  KINDS = %w[auto_response copilot_response regenerate force_talk human_turn].freeze
  ...
  # Note: human_turn? and regenerate? are generated by the enum :kind declaration
  ...
  # Human turns don't execute - they just track state.
  def should_execute?
    !human_turn?
  end
  ...
  class << self
    # Create a human turn run for auto mode tracking.
    def create_human_turn!(conversation:, speaker:, timeout_seconds:, round_id:)
      create!(
