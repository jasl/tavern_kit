# frozen_string_literal: true

require_relative "message"

module TavernKit
  module Prompt
    # A Block represents a single unit in a prompt plan.
    #
    # Blocks are the output of the prompt building process. They contain:
    # - The message content and role for LLM consumption
    # - Metadata for debugging, filtering, and budget management
    # - Positioning information for insertion logic
    #
    # This is the "Extended Block Structure" from the TavernKit roadmap,
    # aligned with SillyTavern's Prompt Item model.
    #
    # @example Creating a simple block
    #   Block.new(role: :system, content: "You are a helpful assistant.")
    #
    # @example Creating a block with full attributes
    #   Block.new(
    #     id: "main-prompt-001",
    #     role: :system,
    #     content: "System prompt",
    #     slot: :main_prompt,
    #     insertion_point: :relative,
    #     priority: 10,
    #     token_budget_group: :system,
    #     tags: [:core, :immutable],
    #   )
    #
    class Block
      # Valid roles for blocks (aligned with chat completion APIs)
      ROLES = %i[system user assistant].freeze

      # Valid insertion points (aligned with Lore::Entry::POSITIONS + relative)
      INSERTION_POINTS = %i[
        relative
        in_chat
        before_char_defs
        after_char_defs
        before_example_messages
        after_example_messages
        top_of_an
        bottom_of_an
        outlet
      ].freeze

      # Token budget groups for eviction decisions.
      # Lower priority groups are evicted first during context trimming.
      # - :system   Main prompt, PHI - never evicted
      # - :examples Example dialogue - evicted first
      # - :lore     World Info - evicted second
      # - :history  Chat history - evicted last
      # - :custom   Custom injections
      # - :default  Fallback group
      BUDGET_GROUPS = %i[system examples lore history custom default].freeze

      attr_reader :id,
                  :role,
                  :content,
                  :name,
                  :slot,
                  :enabled,
                  :insertion_point,
                  :depth,
                  :order,
                  :priority,
                  :token_budget_group,
                  :tags,
                  :metadata

      # @param id [String, nil] Unique identifier (auto-generated if nil)
      # @param role [Symbol] Message role (:system, :user, :assistant)
      # @param content [String] Text content
      # @param name [String, nil] Optional message name (OpenAI-style name field)
      # @param slot [Symbol, nil] Block type identifier (e.g., :main_prompt, :history)
      # @param enabled [Boolean] Whether this block is enabled (default: true)
      # @param insertion_point [Symbol] Where to insert (:relative, :in_chat, etc.)
      # @param depth [Integer] Depth for :in_chat insertion (0 = after last message)
      # @param order [Integer] Ordering within same position (lower = earlier)
      # @param priority [Integer] Budget eviction priority (lower = keep longer)
      # @param token_budget_group [Symbol] Budget group for grouped eviction
      # @param tags [Array<Symbol>] Tags for filtering/hooks
      # @param metadata [Hash] Extension data for custom attributes
      def initialize(
        role:,
        content:,
        id: nil,
        name: nil,
        slot: nil,
        enabled: true,
        insertion_point: :relative,
        depth: 0,
        order: 100,
        priority: 100,
        token_budget_group: :default,
        tags: [],
        metadata: {}
      )
        resolved_id = id || SecureRandom.uuid
        unless resolved_id.is_a?(String)
          raise ArgumentError, "id must be a String (or nil), got: #{resolved_id.class}"
        end
        @id = resolved_id

        unless role.is_a?(Symbol) && ROLES.include?(role)
          raise ArgumentError, "role must be one of #{ROLES.inspect}, got: #{role.inspect}"
        end
        @role = role

        unless content.is_a?(String)
          raise ArgumentError, "content must be a String, got: #{content.class}"
        end
        @content = content

        if !name.nil? && !name.is_a?(String)
          raise ArgumentError, "name must be a String (or nil), got: #{name.class}"
        end
        @name = name

        if !slot.nil? && !slot.is_a?(Symbol)
          raise ArgumentError, "slot must be a Symbol (or nil), got: #{slot.class}"
        end
        @slot = slot

        unless enabled == true || enabled == false
          raise ArgumentError, "enabled must be a Boolean, got: #{enabled.class}"
        end
        @enabled = enabled

        unless insertion_point.is_a?(Symbol) && INSERTION_POINTS.include?(insertion_point)
          raise ArgumentError,
                "insertion_point must be one of #{INSERTION_POINTS.inspect}, got: #{insertion_point.inspect}"
        end
        @insertion_point = insertion_point

        unless depth.is_a?(Integer) && depth >= 0
          raise ArgumentError, "depth must be a non-negative Integer, got: #{depth.inspect}"
        end
        @depth = depth

        unless order.is_a?(Integer)
          raise ArgumentError, "order must be an Integer, got: #{order.class}"
        end
        @order = order

        unless priority.is_a?(Integer)
          raise ArgumentError, "priority must be an Integer, got: #{priority.class}"
        end
        @priority = priority

        unless token_budget_group.is_a?(Symbol) && BUDGET_GROUPS.include?(token_budget_group)
          raise ArgumentError,
                "token_budget_group must be one of #{BUDGET_GROUPS.inspect}, got: #{token_budget_group.inspect}"
        end
        @token_budget_group = token_budget_group

        unless tags.is_a?(Array) && tags.all? { |t| t.is_a?(Symbol) }
          raise ArgumentError, "tags must be an Array<Symbol>, got: #{tags.class}"
        end
        @tags = tags.dup.freeze

        unless metadata.is_a?(Hash)
          raise ArgumentError, "metadata must be a Hash, got: #{metadata.class}"
        end
        @metadata = metadata.dup
      end

      # @return [Boolean] whether this block is enabled
      def enabled?
        @enabled
      end

      # @return [Boolean] whether this block is disabled
      def disabled?
        !@enabled
      end

      # @return [Boolean] whether this block should be inserted in-chat
      def in_chat?
        @insertion_point == :in_chat
      end

      # @return [Boolean] whether this block uses relative positioning
      def relative?
        @insertion_point == :relative
      end

      # Convert to a Message for LLM consumption.
      # @return [Message]
      def to_message
        Message.new(role: role, content: content, name: name)
      end

      # Serialize to a hash for debugging/inspection.
      # @return [Hash]
      def to_h
        {
          id: id,
          role: role,
          content: content,
          name: name,
          slot: slot,
          enabled: enabled,
          insertion_point: insertion_point,
          depth: depth,
          order: order,
          priority: priority,
          token_budget_group: token_budget_group,
          tags: tags,
          metadata: metadata,
        }
      end

      # Create a new Block with modified attributes.
      # @param attrs [Hash] attributes to override
      # @return [Block] new block with merged attributes
      def with(**attrs)
        Block.new(**to_h.merge(attrs))
      end

      # Create a disabled copy of this block.
      # @return [Block]
      def disable
        with(enabled: false)
      end

      # Create an enabled copy of this block.
      # @return [Block]
      def enable
        with(enabled: true)
      end
    end
  end
end
