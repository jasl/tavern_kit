...
module TavernKit
  module Prompt
    # Models a single SillyTavern Prompt Manager "Prompt Entry".
    #
    # - Pinned prompts: built-in placeholders that map to TavernKit's internal
    ...
    class PromptEntry
      ...
      #
      # - Empty triggers array: activates for ALL generation types (default ST behavior)
      # - Non-empty triggers array: only activates if generation_type is in the array
      ...
      def evaluate_conditions(cond_hash, context)
        ...
        # Leaf condition families (AND semantics)
      ...
      def attribute_matches?(pattern, value)
        ...
        if (re = JsRegexToRuby.try_convert(p, literal_only: true))
          return re.match?(value.to_s)
        end

        value.to_s.strip.casecmp?(p)
      end
